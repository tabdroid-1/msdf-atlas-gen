...
static bool is8bitFormat(Format format) {
  return format == PNG || format == BMP || format == RGBA || format == TEXT ||
         format == BINARY;
}
  ...
  for (const char *in = edgeAssignment; *in; ++in) {
    ...
    case 'y':
      ...
      if (e < contour->edges.size()) {
        contour->edges[e]->color =
            EdgeColor((*in == 'C' || *in == 'c') * CYAN |
                      (*in == 'M' || *in == 'm') * MAGENTA |
                      (*in == 'Y' || *in == 'y') * YELLOW |
                      (*in == 'W' || *in == 'w') * WHITE);
        change = true;
...
static bool writeTextBitmap(FILE *file, const float *values, int cols,
                            int rows) {
...
static bool writeTextBitmapFloat(FILE *file, const float *values, int cols,
                                 int rows) {
  for (int row = 0; row < rows; ++row) {
    for (int col = 0; col < cols; ++col) {
...
template <int N>
static const char *writeOutput(const BitmapConstRef<float, N> &bitmap,
                               const char *filename, Format &format) {
  if (filename) {
    if (format == AUTO) {
#if defined(MSDFGEN_EXTENSIONS) && !defined(MSDFGEN_DISABLE_PNG)
...
#endif
      else if (cmpExtension(filename, ".bmp"))
        format = BMP;
      else if (cmpExtension(filename, ".tiff") ||
               cmpExtension(filename, ".tif"))
        format = TIFF;
      else if (cmpExtension(filename, ".rgba"))
        format = RGBA;
      else if (cmpExtension(filename, ".fl32"))
        format = FL32;
      else if (cmpExtension(filename, ".txt"))
        format = TEXT;
      else if (cmpExtension(filename, ".bin"))
        format = BINARY;
      else
...
#if defined(MSDFGEN_EXTENSIONS) && !defined(MSDFGEN_DISABLE_PNG)
    case PNG:
      return savePng(bitmap, filename) ? NULL
                                       : "Failed to write output PNG image.";
#endif
    case BMP:
      return saveBmp(bitmap, filename) ? NULL
                                       : "Failed to write output BMP image.";
    case TIFF:
      return saveTiff(bitmap, filename) ? NULL
                                        : "Failed to write output TIFF image.";
    case RGBA:
      return saveRgba(bitmap, filename) ? NULL
                                        : "Failed to write output RGBA image.";
    case FL32:
      return saveFl32(bitmap, filename) ? NULL
                                        : "Failed to write output FL32 image.";
    case TEXT:
    case TEXT_FLOAT: {
      FILE *file = fopen(filename, "w");
      if (!file)
        return "Failed to write output text file.";
      if (format == TEXT)
        writeTextBitmap(file, bitmap.pixels, N * bitmap.width, bitmap.height);
      else if (format == TEXT_FLOAT)
        writeTextBitmapFloat(file, bitmap.pixels, N * bitmap.width,
                             bitmap.height);
      fclose(file);
      return NULL;
    }
    ...
    if (format == AUTO || format == TEXT)
      ...
      writeTextBitmapFloat(stdout, bitmap.pixels, N * bitmap.width,
                           bitmap.height);
    else
...
#if defined(MSDFGEN_EXTENSIONS) &&                                             \
    (defined(MSDFGEN_DISABLE_SVG) || defined(MSDFGEN_DISABLE_PNG) ||           \
     defined(MSDFGEN_DISABLE_VARIABLE_FONTS))
...
static const char *const helpText =
    "\n"
    "Multi-channel signed distance field generator by Viktor Chlumsky "
    "v" MSDFGEN_VERSION_STRING TITLE_SUFFIX "\n"
    "-----------------------------------------------------------------"
    ...
    "  msdf - Generate multi-channel signed distance field. This is used by "
    "default if no mode is specified.\n"
    "  mtsdf - Generate combined multi-channel and true signed distance field "
    ...
    "\tFormat of character code is '?', 63, 0x3F (Unicode value), or g34 "
    "(glyph index).\n"
#endif
    ...
    "\tLoads a single glyph from a variable font. Specify variable values as "
    "x.ttf?var1=0.5&var2=1\n"
#endif
    "\n"
    ...
    "\tSpecifies the outermost (negative) and innermost representable distance "
    "in pixels.\n"
    ...
    "  -autoframe\n"
    "\tAutomatically scales (unless specified) and translates the shape to "
    "fit.\n"
...
#ifdef MSDFGEN_EXTENSIONS
    "  -emnormalize\n"
    "\tBefore applying scale, normalizes font glyph coordinates so that 1 = 1 "
    "em.\n"
#endif
    "  -errorcorrection <mode>\n"
    "\tChanges the MSDF/MTSDF error correction mode. Use -errorcorrection help "
    ...
    "\tSets the minimum ratio between the pre-correction distance error and "
    ...
    "\tSaves the shape description into a text file that can be edited and "
    "loaded using -shapedesc.\n"
...
#if defined(MSDFGEN_EXTENSIONS) && !defined(MSDFGEN_DISABLE_PNG)
    "  -format <png / bmp / tiff / rgba / fl32 / text / textfloat / bin / "
    "binfloat / binfloatbe>\n"
#else
    "  -format <bmp / tiff / rgba / fl32 / text / textfloat / bin / binfloat / "
    "binfloatbe>\n"
#endif
    "\tSpecifies the output format of the distance field. Otherwise it is "
    "chosen based on output file extension.\n"
    "  -guessorder\n"
    "\tAttempts to detect if shape contours have the wrong winding and "
    "generates the SDF with the right one.\n"
    "  -help\n"
    "\tDisplays this help.\n"
    "  -legacy\n"
...
#ifdef MSDFGEN_EXTENSIONS
    "  -noemnormalize\n"
    "\tRaw integer font glyph coordinates will be used. Without this option, "
    "legacy scaling will be applied.\n"
#endif
#ifdef MSDFGEN_USE_SKIA
    "  -nopreprocess\n"
    "\tDisables path preprocessing which resolves self-intersections and "
...
#endif
    "  -o <filename>\n"
    "\tSets the output file name. The default value is "
    "\"output." DEFAULT_IMAGE_EXTENSION "\".\n"
#ifdef MSDFGEN_USE_SKIA
    "  -overlap\n"
    "\tSwitches to distance field generator with support for overlapping "
    "contours.\n"
#endif
    "  -printmetrics\n"
    "\tPrints relevant metrics of the shape to the standard output.\n"
    "  -pxrange <range>\n"
    "\tSets the width of the range between the lowest and highest signed "
    "distance in pixels.\n"
    "  -range <range>\n"
    "\tSets the width of the range between the lowest and highest signed "
    "distance in shape units.\n"
    "  -reverseorder\n"
...
#ifdef MSDFGEN_USE_SKIA
    "  -scanline\n"
    "\tPerforms an additional scanline pass to fix the signs of the "
    "distances.\n"
#endif
    "  -seed <n>\n"
    "\tSets the random seed for edge coloring heuristic.\n"
    "  -stdout\n"
    "\tPrints the output instead of storing it in a file. Only text formats "
    "are supported.\n"
    "  -testrender <filename." DEFAULT_IMAGE_EXTENSION "> <width> <height>\n"
#if defined(MSDFGEN_EXTENSIONS) && !defined(MSDFGEN_DISABLE_PNG)
    "\tRenders an image preview using the generated distance field and saves "
    "it as a PNG file.\n"
#else
    "\tRenders an image preview using the generated distance field and saves "
    "it as a TIFF file.\n"
#endif
    "  -testrendermulti <filename." DEFAULT_IMAGE_EXTENSION
    "> <width> <height>\n"
    ...
    "  -windingpreprocess\n"
    "\tAttempts to fix only the contour windings assuming no "
    "self-intersections and even-odd fill rule.\n"
    "  -yflip\n"
    "\tInverts the Y axis in the output distance field. The default order is "
    "bottom to top.\n"
    "\n";
...
static const char *errorCorrectionHelpText =
    ...
    "  auto-fast\n"
    "\tDetects inversion artifacts and distance errors that do not affect "
    "edges by range testing.\n"
    "  auto-full\n"
    "\tDetects inversion artifacts and distance errors that do not affect "
    "edges by exact distance evaluation.\n"
    "  auto-mixed (default)\n"
    "\tDetects inversions by distance evaluation and distance errors that do "
    "not affect edges by range testing.\n"
    "  disabled\n"
    "\tDisables error correction.\n"
    "  distance-fast\n"
    "\tDetects distance errors by range testing. Does not care if edges and "
    "corners are affected.\n"
    "  distance-full\n"
    "\tDetects distance errors by exact distance evaluation. Does not care if "
    "edges and corners are affected, slow.\n"
    "  edge-fast\n"
...
#define ABORT(msg)                                                             \
  ...
  enum { SINGLE, PERPENDICULAR, MULTI, MULTI_AND_TRUE, METRICS } mode = MULTI;
  enum {
    NO_PREPROCESS,
    WINDING_PREPROCESS,
    FULL_PREPROCESS
  ...
  enum { RANGE_UNIT, RANGE_PX } rangeMode = RANGE_PX;
  Range range(1);
  Range pxRange(2);
  Vector2 translate;
  Vector2 scale = 1;
  ...
  enum { KEEP, REVERSE, GUESS } orientation = KEEP;
  unsigned long long coloringSeed = 0;
  void (*edgeColoring)(Shape &, double, unsigned long long) =
      &edgeColoringSimple;
...
#define ARG_CASE(s, p)                                                         \
  if ((!strcmp(arg, s)) && argPos + (p) < argc && (++argPos, true))
...
#define SET_FORMAT(fmt, ext)                                                   \
  do {                                                                         \
    ...
    ARG_CASE("-o" ARG_CASE_OR "-out" ARG_CASE_OR "-output" ARG_CASE_OR
             "-imageout",
             1) {
      output = argv[argPos++];
    ...
    ARG_CASE("-fillrule", 1) {
      ...
      else if (ARG_IS("evenodd") || ARG_IS("odd"))
        fillRule = FILL_ODD;
      else if (ARG_IS("positive"))
        fillRule = FILL_POSITIVE;
      else if (ARG_IS("negative"))
        fillRule = FILL_NEGATIVE;
      else
    ...
    ARG_CASE("-dimensions" ARG_CASE_OR "-size", 2) {
      unsigned w, h;
      if (!(parseUnsigned(w, argv[argPos++]) &&
            parseUnsigned(h, argv[argPos++]) && w && h))
        ABORT("Invalid dimensions. Use -dimensions <width> <height> with two "
              "positive integers.");
      width = w, height = h;
      continue;
    }
    ...
    ARG_CASE("-pxrange", 1) {
      ...
      if (!parseDouble(r, argv[argPos++]))
        ABORT(
            "Invalid range argument. Use -pxrange <range> with a real number.");
      if (r == 0)
        ABORT("Range must be non-zero.");
      rangeMode = RANGE_PX;
      pxRange = Range(r);
    ...
    ARG_CASE("-arange" ARG_CASE_OR "-aunitrange", 2) {
      ...
      if (!(parseDouble(r0, argv[argPos++]) && parseDouble(r1, argv[argPos++])))
        ABORT("Invalid range arguments. Use -arange <minimum> <maximum> with "
              "two real numbers.");
      if (r0 == r1)
    ...
    ARG_CASE("-apxrange", 2) {
      double r0, r1;
      if (!(parseDouble(r0, argv[argPos++]) && parseDouble(r1, argv[argPos++])))
        ABORT("Invalid range arguments. Use -apxrange <minimum> <maximum> with "
              "two real numbers.");
      if (r0 == r1)
        ABORT("Range must be non-empty.");
      rangeMode = RANGE_PX;
    ...
    ARG_CASE("-scale", 1) {
      ...
      if (!(parseDouble(s, argv[argPos++]) && s > 0))
        ABORT("Invalid scale argument. Use -scale <scale> with a positive real "
              "number.");
      scale = s;
      scaleSpecified = true;
      continue;
    ...
    ARG_CASE("-ascale", 2) {
      double sx, sy;
      if (!(parseDouble(sx, argv[argPos++]) &&
            parseDouble(sy, argv[argPos++]) && sx > 0 && sy > 0))
        ABORT("Invalid scale arguments. Use -ascale <x> <y> with two positive "
              "real numbers.");
      scale.set(sx, sy);
      scaleSpecified = true;
      continue;
    ...
    ARG_CASE("-translate", 2) {
      ...
      if (!(parseDouble(tx, argv[argPos++]) && parseDouble(ty, argv[argPos++])))
        ABORT("Invalid translate arguments. Use -translate <x> <y> with two "
              "real numbers.");
      translate.set(tx, ty);
      continue;
    }
    ...
    ARG_CASE("-errorcorrection", 1) {
      if (ARG_IS("disable") || ARG_IS("disabled") || ARG_IS("0") ||
          ARG_IS("none") || ARG_IS("false")) {
      ...
      } else if (ARG_IS("default") || ARG_IS("auto") || ARG_IS("auto-mixed") ||
                 ARG_IS("mixed")) {
        generatorConfig.errorCorrection.mode =
      ...
      } else if (ARG_IS("distance") || ARG_IS("distance-fast") ||
                 ARG_IS("indiscriminate") || ARG_IS("indiscriminate-fast")) {
        generatorConfig.errorCorrection.mode =
        ...
        fputs("Unknown error correction mode. Use -errorcorrection help for "
              "more information.\n",
              stderr);
      ++argPos;
    ...
    ARG_CASE("-edgecolors", 1) {
      ...
      for (int i = 0; argv[argPos][i]; ++i) {
        ...
        ABORT(
            "Invalid edge coloring sequence. Use -edgecolors <color sequence> "
            "with only the colors C, M, Y, and W. Separate contours by commas "
            "and use ? to keep the default assigment for a contour.");
      EDGE_COLOR_VERIFIED:;
      }
    ...
    ARG_CASE("-distanceshift", 1) {
      ...
      if (!parseDouble(ds, argv[argPos++]))
        ABORT("Invalid distance shift. Use -distanceshift <shift> with a real "
              "value.");
    ...
    ARG_CASE("-testrender", 3) {
      ...
      if (!(parseUnsigned(w, argv[argPos++]) &&
            parseUnsigned(h, argv[argPos++]) && (int)w > 0 && (int)h > 0))
        ABORT("Invalid arguments for test render. Use -testrender "
              "<output." DEFAULT_IMAGE_EXTENSION "> <width> <height>.");
    ...
    ARG_CASE("-testrendermulti", 3) {
      ...
      if (!(parseUnsigned(w, argv[argPos++]) &&
            parseUnsigned(h, argv[argPos++]) && (int)w > 0 && (int)h > 0))
        ABORT("Invalid arguments for test render. Use -testrendermulti "
              "<output." DEFAULT_IMAGE_EXTENSION "> <width> <height>.");
  ...
  if (!inputType || !input) {
    ...
    ABORT("No input specified! Use -font <file.ttf/otf> <character code> or "
          "see -help.");
#else
    ABORT("No input specified! Use either -svg <file.svg> or -font "
          "<file.ttf/otf> <character code>, or see -help.");
#endif
#else
    ABORT("No input specified! See -help.");
#endif
  }
  if (mode == MULTI_AND_TRUE &&
      (format == BMP ||
       (format == AUTO && output && cmpExtension(output, ".bmp"))))
    ABORT("Incompatible image format. A BMP file cannot contain alpha channel, "
          "which is required in mtsdf mode.");
  Shape shape;
  switch (inputType) {
#if defined(MSDFGEN_EXTENSIONS) && !defined(MSDFGEN_DISABLE_SVG)
  case SVG: {
    ...
    if (svgImportFlags & SVG_IMPORT_INCOMPLETE_FLAG)
      fputs("Warning: SVG file contains multiple paths or shapes but this "
            "version is only able to load one.\n",
            stderr);
    else if (svgImportFlags & SVG_IMPORT_UNSUPPORTED_FEATURE_FLAG)
      fputs(
          "Warning: SVG file likely contains elements that are unsupported.\n",
          stderr);
  ...
  case VAR_FONT: {
    if (!glyphIndexSpecified && !unicode)
      ABORT(
          "No character specified! Use -font <file.ttf/otf> <character code>. "
          "Character code can be a Unicode index (65, 0x41), a character in "
          "apostrophes ('A'), or a glyph index prefixed by g (g36, g0x24).");
    ...
    std::ifstream font_file(input, std::ios::binary);
    size_t size = font_file.tellg();
    char *buffer = new char[size];
    font_file.seekg(0);
    font_file.read(buffer, size);

    guard.font = loadFontData(guard.ft, (byte *)buffer, size);

    if (!guard.font)
      ABORT("Failed to load font file.");
    if (unicode)
      getGlyphIndex(glyphIndex, guard.font, unicode);
    if (!loadGlyph(shape, guard.font, glyphIndex, fontCoordinateScaling,
                   &glyphAdvance))
      ABORT("Failed to load glyph from font file.");
    if (!fontCoordinateScalingSpecified &&
        (!autoFrame || scaleSpecified || rangeMode == RANGE_UNIT ||
         mode == METRICS || printMetrics || shapeExport || svgExport)) {
      fputs("Warning: Using legacy font coordinate conversion for "
            "compatibility reasons.\n"
            "         The implicit scaling behavior will likely change in a "
            "future version resulting in different output.\n"
            "         To silence this warning, use one of the following "
            "options:\n"
            "           -noemnormalize to switch to the correct native font "
            "coordinates,\n"
            "           -emnormalize to switch to coordinates normalized to 1 "
            "em, or\n"
            "           -legacyfontscaling to keep current behavior and make "
            "sure it will not change.\n",
            stderr);
    }
    break;
  }
  ...
  case FULL_PREPROCESS:
    ...
    if (!resolveShapeGeometry(shape))
      ...
      skipColoring = false;
      fputs("Note: Input shape coloring won't be preserved due to geometry "
            "preprocessing.\n",
            stderr);
    }
#else
    ABORT("Shape geometry preprocessing (-preprocess) is not available in this "
          "version because the Skia library is not present.");
#endif
    break;
  }
  shape.normalize();
  ...
  if (outputDistanceShift) {
    ...
    double rangeShift =
        -outputDistanceShift * (rangeRef.upper - rangeRef.lower);
  ...
  if (mode == METRICS || printMetrics) {
    ...
    if (svgViewBox.l < svgViewBox.r && svgViewBox.b < svgViewBox.t)
      fprintf(out, "view box = %.17g, %.17g, %.17g, %.17g\n", svgViewBox.l,
              svgViewBox.b, svgViewBox.r, svgViewBox.t);
  ...
  if (scanlinePass) {
    if (explicitErrorCorrectionMode &&
      ...
      fprintf(stderr,
              "Selected error correction mode not compatible with scanline "
              "pass, falling back to %s.\n",
              fallbackModeName);
    }
    generatorConfig.errorCorrection.mode = ErrorCorrectionConfig::DISABLED;
  ...
  case MULTI: {
    ...
    if (legacyMode)
      generateMSDF_legacy(msdf, shape, range, scale, translate,
                          generatorConfig.errorCorrection);
    else
  ...
  case MULTI_AND_TRUE: {
    ...
    if (legacyMode)
      generateMTSDF_legacy(mtsdf, shape, range, scale, translate,
                           generatorConfig.errorCorrection);
    else
  ...
  if (scanlinePass) {
    ...
    case MULTI:
      ...
      msdfErrorCorrection(msdf, shape, transformation,
                          postErrorCorrectionConfig);
      break;
    case MULTI_AND_TRUE:
  ...
  case PERPENDICULAR:
    ...
    if (is8bitFormat(format) &&
        (testRenderMulti || testRender || estimateError))
      simulate8bit(sdf);
    if (estimateError) {
      double sdfError = estimateSDFError(
          sdf, shape, transformation, SDF_ERROR_ESTIMATE_PRECISION, fillRule);
      printf("SDF error ~ %e\n", sdfError);
    }
    if (testRenderMulti) {
  ...
  case MULTI:
    ...
    if (estimateError) {
      double sdfError = estimateSDFError(
          msdf, shape, transformation, SDF_ERROR_ESTIMATE_PRECISION, fillRule);
      printf("SDF error ~ %e\n", sdfError);
    }
    if (testRenderMulti) {
      Bitmap<float, 3> render(testWidthM, testHeightM);
  ...
  case MULTI_AND_TRUE:
    ...
    if (estimateError) {
      double sdfError = estimateSDFError(
          mtsdf, shape, transformation, SDF_ERROR_ESTIMATE_PRECISION, fillRule);
      printf("SDF error ~ %e\n", sdfError);
    }
    if (testRenderMulti) {
