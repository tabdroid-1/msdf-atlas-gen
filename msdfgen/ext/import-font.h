...
struct FontVariationAxis {
  ...
  double maxValue;
  /// The axis's default coordinate value. FreeType computes meaningful default
...
enum FontCoordinateScaling {
  ...
  /// The incorrect legacy version that was in effect before version 1.12,
  /// coordinate values are divided by 64 - DO NOT USE - for backwards
  /// compatibility only
  FONT_SCALING_LEGACY
};
...
/// Creates a FontHandle from FT_Face that was loaded by the user. destroyFont
/// must still be called but will not affect the FT_Face.
FontHandle *adoptFreetypeFont(FT_Face ftFace);
/// Converts the geometry of FreeType's FT_Outline to a Shape object.
FT_Error
readFreetypeOutline(Shape &output, FT_Outline *outline,
                    double scale = MSDFGEN_LEGACY_FONT_COORDINATE_SCALE);
#endif
...
bool getFontMetrics(
    ...
    FontCoordinateScaling coordinateScaling = FONT_SCALING_LEGACY);
/// Outputs the width of the space and tab characters.
bool getFontWhitespaceWidth(
    double &spaceAdvance, double &tabAdvance, FontHandle *font,
    FontCoordinateScaling coordinateScaling = FONT_SCALING_LEGACY);
...
bool loadGlyph(Shape &output, FontHandle *font, GlyphIndex glyphIndex,
               ...
               double *outAdvance = NULL, unicode_t *outUnicode = NULL);
bool loadGlyph(Shape &output, FontHandle *font, unicode_t unicode,
               FontCoordinateScaling coordinateScaling,
               double *outAdvance = NULL);
