...
bool exportJSON(const FontGeometry *fonts, int fontCount, ImageType imageType,
                const JsonAtlasMetrics &metrics, const char *filename,
                bool kerning) {
  ...
  fputs("\"atlas\":{", f);
  {
    ...
    if (imageType == ImageType::SDF || imageType == ImageType::PSDF ||
      ...
      fprintf(f, "\"distanceRange\":%.17g,",
              metrics.distanceRange.upper - metrics.distanceRange.lower);
      fprintf(f, "\"distanceRangeMiddle\":%.17g,",
              .5 * (metrics.distanceRange.lower + metrics.distanceRange.upper));
    }
    fprintf(f, "\"size\":%.17g,", metrics.size);
    fprintf(f, "\"width\":%d,", metrics.width);
    fprintf(f, "\"height\":%d,", metrics.height);
    fprintf(f, "\"yOrigin\":\"%s\"",
            metrics.yDirection == YDirection::TOP_DOWN ? "top" : "bottom");
    if (metrics.grid) {
      fputs(",\"grid\":{", f);
      ...
      if (metrics.grid->originY) {
        ...
        case YDirection::TOP_DOWN:
          fprintf(f, ",\"originY\":%.17g",
                  (metrics.grid->cellHeight - metrics.grid->spacing - 1) /
                          metrics.size -
                      *metrics.grid->originY);
          break;
        }
      }
      fputs("}", f);
  ...
  for (int i = 0; i < fontCount; ++i) {
    ...
    fputs("\"metrics\":{", f);
    {
      ...
      fprintf(f, "\"underlineThickness\":%.17g",
      ...
      if (l || b || r || t) {
        ...
        case YDirection::TOP_DOWN:
          fprintf(f,
                  ...
                  "17g,\"bottom\":%.17g}",
                  l, metrics.height - t, r, metrics.height - b);
          break;
        }
      }
      fputs("}", f);
