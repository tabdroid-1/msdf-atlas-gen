...
/*
 * MULTI-CHANNEL SIGNED DISTANCE FIELD ATLAS GENERATOR - standalone console
 * program
 * --------------------------------------------------------------------------------
...
static const char *const helpText =
    R"(
MSDF Atlas Generator by Viktor Chlumsky v)" MSDF_ATLAS_VERSION_STRING
    R"( (with MSDFgen v)" MSDFGEN_VERSION_STRING TITLE_SUFFIX R"()
----------------------------------------------------------------)" VERSION_UNDERLINE EXTRA_UNDERLINE
    R"(
...
#define ARG_PREFIX(s) strStartsWith(argv[argPos], s)
...
#ifndef MSDFGEN_DISABLE_PNG
        ABORT("Invalid image format. Valid formats are: png, bmp, tiff, rgba, "
              "fl32, text, textfloat, bin, binfloat");
#else
        ABORT("Invalid image format. Valid formats are: bmp, tiff, rgba, fl32, "
              "text, textfloat, bin, binfloat");
#endif
      }
      imageFormatName = arg;
      ++argPos;
    ...
    ARG_CASE("-and", 0) {
      if (!fontInput.fontFilename && !fontInput.charsetFilename &&
          !fontInput.charsetString && fontInput.fontScale < 0)
    ...
    ARG_CASE("-emrange", 1) {
      ...
      if (!(parseDouble(r, argv[argPos++]) && r != 0))
        ABORT("Invalid range argument. Use -emrange <em range> with a non-zero "
              "real number.");
    ...
    ARG_CASE("-aemrange", 2) {
      ...
      if (!(parseDouble(r0, argv[argPos++]) && parseDouble(r1, argv[argPos++])))
        ABORT("Invalid range arguments. Use -aemrange <minimum> <maximum> with "
              "two real numbers.");
      if (r0 == r1)
    ...
    ARG_CASE("-pxalign", 1) {
      if (ARG_IS("off") || ARG_PREFIX("disable") || ARG_IS("0") ||
          ARG_IS("false") || ARG_PREFIX("n"))
        config.pxAlignOriginX = false, config.pxAlignOriginY = false;
      else if (ARG_IS("on") || ARG_PREFIX("enable") || ARG_IS("1") ||
               ARG_IS("true") || ARG_IS("hv") || ARG_PREFIX("y"))
        ...
        ABORT("Unknown -pxalign setting. Use one of: off, on, horizontal, "
              "vertical.");
      ++argPos;
      continue;
    }
    ARG_CASE("-empadding", 1) {
      ...
      if (!parseDouble(p, argv[argPos++]))
        ABORT("Invalid padding argument. Use -empadding <padding> with a real "
              "number.");
    ...
    ARG_CASE("-pxpadding", 1) {
      double p;
      if (!parseDouble(p, argv[argPos++]))
        ABORT("Invalid padding argument. Use -pxpadding <padding> with a real "
              "number.");
    ...
    ARG_CASE("-outerempadding", 1) {
      double p;
      if (!parseDouble(p, argv[argPos++]))
        ABORT("Invalid padding argument. Use -outerempadding <padding> with a "
              "real number.");
    ...
    ARG_CASE("-outerpxpadding", 1) {
      double p;
      if (!parseDouble(p, argv[argPos++]))
        ABORT("Invalid padding argument. Use -outerpxpadding <padding> with a "
              "real number.");
    ...
    ARG_CASE("-aempadding", 4) {
      double l, b, r, t;
      if (!(parseDouble(l, argv[argPos++]) && parseDouble(b, argv[argPos++]) &&
            parseDouble(r, argv[argPos++]) && parseDouble(t, argv[argPos++])))
        ABORT("Invalid padding arguments. Use -aempadding <left> <bottom> "
              "<right> <top> with 4 real numbers.");
      innerPaddingUnits = Units::EMS;
      innerPadding.l = l, innerPadding.b = b, innerPadding.r = r,
      innerPadding.t = t;
      continue;
    }
    ARG_CASE("-apxpadding", 4) {
      ...
      if (!(parseDouble(l, argv[argPos++]) && parseDouble(b, argv[argPos++]) &&
            parseDouble(r, argv[argPos++]) && parseDouble(t, argv[argPos++])))
        ABORT("Invalid padding arguments. Use -apxpadding <left> <bottom> "
              "<right> <top> with 4 real numbers.");
      innerPaddingUnits = Units::PIXELS;
      innerPadding.l = l, innerPadding.b = b, innerPadding.r = r,
      innerPadding.t = t;
    ...
    ARG_CASE("-aouterempadding", 4) {
      ...
      if (!(parseDouble(l, argv[argPos++]) && parseDouble(b, argv[argPos++]) &&
            parseDouble(r, argv[argPos++]) && parseDouble(t, argv[argPos++])))
        ABORT("Invalid padding arguments. Use -aouterempadding <left> <bottom> "
              "<right> <top> with 4 real numbers.");
      outerPaddingUnits = Units::EMS;
      outerPadding.l = l, outerPadding.b = b, outerPadding.r = r,
      outerPadding.t = t;
      continue;
    }
    ARG_CASE("-aouterpxpadding", 4) {
      ...
      if (!(parseDouble(l, argv[argPos++]) && parseDouble(b, argv[argPos++]) &&
            parseDouble(r, argv[argPos++]) && parseDouble(t, argv[argPos++])))
        ABORT("Invalid padding arguments. Use -aouterpxpadding <left> <bottom> "
              "<right> <top> with 4 real numbers.");
      outerPaddingUnits = Units::PIXELS;
      outerPadding.l = l, outerPadding.b = b, outerPadding.r = r,
      outerPadding.t = t;
    ...
    ARG_CASE("-uniformcols", 1) {
      ...
      if (!(parseUnsigned(c, argv[argPos++]) && c))
        ABORT("Invalid number of grid columns. Use -uniformcols <N> with a "
              "positive integer.");
      config.grid.cols = c;
      continue;
    }
    ARG_CASE("-uniformcell", 2) {
      ...
      if (!(parseUnsigned(w, argv[argPos++]) &&
            parseUnsigned(h, argv[argPos++]) && w && h))
        ABORT("Invalid cell dimensions. Use -uniformcell <width> <height> with "
              "two positive integers.");
    ...
    ARG_CASE("-uniformcellconstraint", 1) {
      ...
      if (ARG_IS("none") || ARG_IS("rect"))
        ...
        ABORT("Unknown dimensions constaint. Use -uniformcellconstraint with "
              "one of: none, pots, potr, square, square2, or square4.");
      ++argPos;
      continue;
    }
    ARG_CASE("-uniformorigin", 1) {
      ...
      if (ARG_IS("off") || ARG_PREFIX("disable") || ARG_IS("0") ||
        ...
        ABORT("Unknown -uniformorigin setting. Use one of: off, on, "
              "horizontal, vertical.");
      ++argPos;
      continue;
    }
  ...
  for (std::vector<FontInput>::reverse_iterator it = fontInputs.rbegin();
    ...
    if (!(it->charsetFilename || it->charsetString ||
          it->glyphIdentifierType == GlyphIdentifierType::GLYPH_INDEX) &&
        (nextFontInput->charsetFilename || nextFontInput->charsetString ||
         nextFontInput->glyphIdentifierType ==
  ...
  if (packingStyle == PackingStyle::TIGHT &&
      atlasSizeConstraint == DimensionsConstraint::NONE)
    atlasSizeConstraint = DimensionsConstraint::MULTIPLE_OF_FOUR_SQUARE;
  ...
  if (!(fixedWidth > 0 && fixedHeight > 0) &&
      !(fixedCellWidth > 0 && fixedCellHeight > 0) && !(minEmSize > 0)) {
    fputs("Neither atlas size nor glyph size selected, using default...\n",
          stderr);
    minEmSize = DEFAULT_SIZE;
  }
  if (config.imageType == ImageType::HARD_MASK ||
      config.imageType == ImageType::SOFT_MASK) {
    rangeUnits = Units::PIXELS;
    rangeValue = 1;
...
#else
      fputs("Warning: You are using a version of this program without PNG "
            "image support!\n",
            stderr);
#endif
    } else if (cmpExtension(config.imageFilename, ".bmp"))
      imageExtension = ImageFormat::BMP;
    else if (cmpExtension(config.imageFilename, ".tiff") ||
             cmpExtension(config.imageFilename, ".tif"))
      imageExtension = ImageFormat::TIFF;
    else if (cmpExtension(config.imageFilename, ".rgba"))
      imageExtension = ImageFormat::RGBA;
    else if (cmpExtension(config.imageFilename, ".fl32"))
      imageExtension = ImageFormat::FL32;
  ...
  if (config.imageFormat == ImageFormat::UNSPECIFIED) {
    ...
    if (!config.arteryFontFilename) {
      if (imageExtension != ImageFormat::UNSPECIFIED)
        ...
        fprintf(stderr,
                "Warning: Could not infer image format from file extension, %s "
                "will be used.\n",
                imageFormatName);
    }
  }
  ...
  if (config.arteryFontFilename &&
    ...
    result = 1;
    fputs("Error: Unable to create an Artery Font file with the specified "
          "image format!\n",
          stderr);
  ...
  bool floatingPointFormat =
      (config.imageFormat == ImageFormat::TIFF ||
       ...
       config.imageFormat == ImageFormat::BINARY_FLOAT ||
       config.imageFormat == ImageFormat::BINARY_FLOAT_BE);
  // TODO: In this case (if spacing is -1), the border pixels of each glyph are
  // black, but still computed. For floating-point output, this may play a role.
  int spacing = config.imageType == ImageType::MSDF ||
                        config.imageType == ImageType::MTSDF
                    ? 0
                    : -1;
  ...
  bool anyCodepointsAvailable = false;
  {
    ...
    for (FontInput &fontInput : fontInputs) {
      ...
      if (fontInput.charsetFilename) {
        ...
        if (!charset.parse(fontInput.charsetString,
                           strlen(fontInput.charsetString),
                           fontInput.glyphIdentifierType !=
                               GlyphIdentifierType::UNICODE_CODEPOINT))
          ABORT(fontInput.glyphIdentifierType ==
                        GlyphIdentifierType::GLYPH_INDEX
                    ? "Failed to parse glyph set specification."
                    : "Failed to parse character set specification.");
      } else if (fontInput.glyphIdentifierType ==
                 GlyphIdentifierType::GLYPH_INDEX)
      ...
      case GlyphIdentifierType::GLYPH_INDEX:
        if (allGlyphCount)
          glyphsLoaded = fontGeometry.loadGlyphRange(
              font, fontInput.fontScale, 0, allGlyphCount,
      ...
      printf("Loaded geometry of %d out of %d glyphs", glyphsLoaded,
             (int)(allGlyphCount + charset.size()));
        ...
        case GlyphIdentifierType::GLYPH_INDEX:
          for (unicode_t cp : charset)
            if (!fontGeometry.getGlyph(msdfgen::GlyphIndex(cp)))
              fprintf(stderr, "%c 0x%02X", first ? ((first = false), ':') : ',',
                      cp);
          break;
        case GlyphIdentifierType::UNICODE_CODEPOINT:
          for (unicode_t cp : charset)
  ...
  // Determine final atlas dimensions, scale and range, pack glyphs
  {
    ...
    case PackingStyle::TIGHT: {
      ...
      atlasPacker.setOriginPixelAlignment(config.pxAlignOriginX,
                                          config.pxAlignOriginY);
      ...
      if (int remaining = atlasPacker.pack(glyphs.data(), glyphs.size())) {
        if (remaining < 0) {
          ...
          fprintf(stderr,
                  "Error: Could not fit %d out of %d glyphs into the atlas.\n",
    ...
    case PackingStyle::GRID: {
      ...
      atlasPacker.setFixedOrigin(config.grid.fixedOriginX,
                                 config.grid.fixedOriginY);
      ...
      if (atlasPacker.hasCutoff())
        fputs("Warning: Grid cell too constrained to fully fit all glyphs, "
              "some may be cut off!\n",
              stderr);
      atlasPacker.getDimensions(config.width, config.height);
      if (!(config.width > 0 && config.height > 0))
      ...
      config.emSize = atlasPacker.getScale();
      config.pxRange = atlasPacker.getPixelRange();
      atlasPacker.getCellDimensions(config.grid.cellWidth,
      ...
      if (config.grid.fixedOriginX || config.grid.fixedOriginY) {
        ...
        if (config.grid.fixedOriginY) {
          ...
          case YDirection::TOP_DOWN:
            printf("Y = %.9g",
                   (config.grid.cellHeight - spacing - 1) / config.emSize -
                       uniformOriginY);
            break;
          }
        }
      ...
      printf("Atlas dimensions: %d x %d (%d columns x %d rows)\n", config.width,
             config.height, config.grid.cols, config.grid.rows);
      break;
    }
    }
  }
  ...
  if (config.jsonFilename) {
    ...
    if (packingStyle == PackingStyle::GRID) {
      gridMetrics.cellWidth = config.grid.cellWidth,
      gridMetrics.cellHeight = config.grid.cellHeight;
      gridMetrics.columns = config.grid.cols,
      gridMetrics.rows = config.grid.rows;
      if (config.grid.fixedOriginX)
    ...
    if (exportJSON(fonts.data(), fonts.size(), config.imageType, jsonMetrics,
                   config.jsonFilename, config.kerning))
